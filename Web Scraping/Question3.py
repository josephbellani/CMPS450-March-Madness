...
# Reads in the PPG for each team in 2022
...
# Reads in the Opposing team's PPG for each team in 2022
...
# Drops the repetitive columns and Renames the meaningful columns
...
# Reads in the PPG for each team in 2021
...
# Reads in the Opposing team's PPG for each team in 2021
...
# Reads in the PPG for each team in 2019
...
# Reads in the Opposing team's PPG for each team in 2019
...
# Reads in the PPG for each team in 2018
...
# Reads in the Opposing team's PPG for each team in 2018
...
# Reads in the PPG for each team in 2017
...
# Reads in the Opposing team's PPG for each team in 2017
...
# Reads in the PPG for each team in 2016
...
# Reads in the Opposing team's PPG for each team in 2016
...
# Reads in the PPG for each team in 2015
...
# Reads in the Opposing team's PPG for each team in 2015
...
# Reads in the PPG for each team in 2014
...
# Reads in the Opposing team's PPG for each team in 2014
...
# Reads in the PPG for each team in 2013
...
# Reads in the Opposing team's PPG for each team in 2013
...
# Reads in the PPG for each team in 2012
...
# Reads in the Opposing team's PPG for each team in 2012
...
# Reads in team names and conferences of teams in NCAA Tournament for 2022 
...
#Merges ppg2022 and OppPPG2022 as well as standardizing the School Column
...
# Renames the teams that conflict with MMteams2022
...
# Creates a dataframe for all 2022 data
MMteams2022 = pd.merge(MMteams2022, points2022)
MMteams2022['year'] = 2022

# Reads in team names and conferences of teams in NCAA Tournament for 2021
...
#Merges ppg2021 and OppPPG2021 as well as standardizing the School Column
...
# Renames the teams that conflict with MMteams2021
...
# Creates a dataframe for all 2021 data
MMteams2021 = pd.merge(MMteams2021, points2021)
MMteams2021['year'] = 2021

# Reads in team names and conferences of teams in NCAA Tournament for 2019
...
#Merges ppg2019 and OppPPG2019 as well as standardizing the School Column
...
# Renames the teams that conflict with MMteams2019
...
# Creates a dataframe for all 2018 data
MMteams2019 = pd.merge(MMteams2019, points2019)
MMteams2019['year'] = 2019

# Reads in team names and conferences of teams in NCAA Tournament for 2018
...
#Merges ppg2018 and OppPPG2018 as well as standardizing the School Column
...
# Renames the teams that conflict with MMteams2018
...
# Reads in team names and conferences of teams in NCAA Tournament for 2017
...
#Merges ppg2017 and OppPPG2017 as well as standardizing the School Column
...
# Renames the teams that conflict with MMteams2017
...
# Creates a dataframe for all 2017 data
MMteams2017 = pd.merge(MMteams2017, points2017)
MMteams2017['year'] = 2017

# Reads in team names and conferences of teams in NCAA Tournament for 2016
...
#Merges ppg2016 and OppPPG2016 as well as standardizing the School Column
...
# Renames the teams that conflict with MMteams2016
...
# Creates a dataframe for all 2016 data
MMteams2016 = pd.merge(MMteams2016, points2016)
MMteams2016['year'] = 2016

# Reads in team names and conferences of teams in NCAA Tournament for 2015
...
#Merges ppg2015 and OppPPG2015 as well as standardizing the School Column
...
# Renames the teams that conflict with MMteams2015
...
# Creates a dataframe for all 2015 data
MMteams2015 = pd.merge(MMteams2015, points2015)
MMteams2015['year'] = 2015

# Reads in team names and conferences of teams in NCAA Tournament for 2014
...
#Merges ppg2014 and OppPPG2014 as well as standardizing the School Column
...
# Renames the teams that conflict with MMteams2014
...
# Creates a dataframe for all 2014 data
MMteams2014 = pd.merge(MMteams2014, points2014)
MMteams2014['year'] = 2014

# Reads in team names and conferences of teams in NCAA Tournament for 2013
url = "https://en.wikipedia.org/wiki/2013_NCAA_Division_I_men%27s_basketball_tournament"

html = requests.get(url).content
MMteams_list = pd.read_html(html, header=1)
...
#Merges ppg2013 and OppPPG2013 as well as standardizing the School Column
points2013 = ppg2013.merge(oppPpg2013)
points2013 = points2013.rename(columns={'Team': 'School'})
points2013['School'] = points2013['School'].apply(lambda x: x[:-2] + 'State' if x[-3:] == 'St' else x)

# Renames the teams that conflict with MMteams2013
...
# Creates a dataframe for all 2013 data
MMteams2013 = pd.merge(MMteams2013, points2013)
MMteams2013['year'] = 2013

# Reads in team names and conferences of teams in NCAA Tournament for 2012
url = "https://en.wikipedia.org/wiki/2012_NCAA_Division_I_men%27s_basketball_tournament"

html = requests.get(url).content
MMteams_list = pd.read_html(html)

# Resets Column Names since the 2012 webpage gave the tables a header
...
#Merges ppg2012 and OppPPG2012 as well as standardizing the School Column
...
# Renames the teams that conflict with MMteams2012
...
# Creates a dataframe for all 2012 data
MMteams2012 = pd.merge(MMteams2012, points2012)
MMteams2012['year'] = 2012

# Combines all data from 2012-2022
...
# Resets Column names as some did match, and deletes the extra columns
...
# Renames some conferences as different years used different names for the same conference
...
# There is not too significant of a trend on this chart, both averages went up a little in 2017 and 2018, but not too 
# significantly, or long enough to consider it a trend.
